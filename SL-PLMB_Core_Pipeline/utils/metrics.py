"""
metrics.py
Includes: IoU, DSC, Sensitivity, Specificity, HD95, ASSD, ICC + 95% CI
...
def calculate_hd95_assd(pred_mask, gt_mask):
    """
    Full calculation of HD95 (95% Hausdorff Distance) + ASSD (Average Symmetric Surface Distance)
    Returns: hd95, assd
    """
...
def icc21(data):
    ...
    Most commonly used in clinical practice: cross-view / cross-sequence consistency
    Returns: icc, (lb, ub) 95% confidence interval
    """
...
def calculate_intraclass_correlation(data, model="two-way", type="consistency"):
    """
    Unified external interface (complete and unabridged)
    model: 'one-way' → ICC(1,1)
           'two-way' → ICC(2,1) 【default for top journals】
    Returns: icc, (lower_bound, upper_bound)
    """
    if data.ndim != 2:
