...
AHA_RINGS = [
    (0.7, 1.0, 6, 0),    # 基底段：1-6
    (0.4, 0.7, 6, 6),    # 中段：7-12
    (0.1, 0.4, 4, 12),   # 心尖段：13-16
    (0.0, 0.1, 1, 16)    # 心尖帽：17
]
APICAL_ANGLES = [0.0, 3*np.pi/2, np.pi, np.pi/2]  # 心尖段4个节段的角度
...
def main():
    ...
    cv_b = seg_cv_results[method_names[1]] * 100  # 转百分比
    cv_c = seg_cv_results[method_names[2]] * 100
    cv_drop = cv_b - cv_c  # 下降值
    ...
    ax.set_xlabel("AHA 17 心肌节段", fontsize=11, fontweight="bold")
    ax.set_ylabel("跨切面变异系数 CV (%)", fontsize=11, fontweight="bold")
    ax.set_title("融合前后17节段跨切面CV对比", fontsize=13, fontweight="bold")
    ax.set_xticks(x)
    ...
    plot_bullseye(cv_b, axes[0], "融合前 CV (%)", vmin=vmin, vmax=vmax)
    plot_bullseye(cv_c, axes[1], "融合后 CV (%)", vmin=vmin, vmax=vmax)
    plot_bullseye(cv_drop, axes[2], "CV 下降值 (%)", vmin=0, vmax=np.max(cv_drop))
    ...
    print("   - seg_cv_barplot.png：17节段CV对比柱状图")
    print("   - cv_bullseye.png：CV牛眼图（融合前/融合后/下降值）")
