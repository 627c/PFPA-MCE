"""
run_seg_ablation.py - 顶刊级分割消融实验自动化引擎 (6指标满血统计显著性版)
功能：
1. 智能检测变体专属权重，已存在的变体自动跳过训练，绝不重复炼丹。
2. 👑 完美对齐 Table 1，全面评估 6 大指标 (DSC, IoU, Sens, Spec, HD95, ASSD)。
3. 👑 全自动计算各个变体与完整模型 (S7) 的配对t检验 p-value，自动标注星号。
4. 生成详尽的 Markdown 报告与 315 张原图-金标准-预测掩膜融合的可视化图像。
"""
import os
import sys
...
def compute_p_value(arr1, arr2, higher_is_better=True):
    ...
    if np.sum(valid_idx) < 5: return 1.0
...
def load_and_preprocess(img_path, mask_path):
    ...
    if mask is None: raise FileNotFoundError(f"掩码文件无法读取: {mask_path}")
...
class SimpleSegDataset(torch.utils.data.Dataset):
    def __init__(self, img_dir, mask_dir):
        ...
        if len(self.valid_pairs) == 0:
            raise RuntimeError(f"在 {img_dir} 和 {mask_dir} 中没有找到任何有效的图片-掩码对！")
...
def get_valid_val_files(view_dir):
    ...
    for img_file in img_files:
        mask_path = find_mask_file(img_file, mask_dir)
        if mask_path is not None: valid_files.append(img_file)
...
def train_variant(variant_name, params, save_path):
    print(f"\n[{variant_name}] 专属权重不存在，启动自动训练 (确保公平对比)...")
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if patience_counter >= PATIENCE:
            print(f"\n⏹️ 早停触发！连续 {PATIENCE} 轮验证集性能没有提升"); break
            
    print(f"✅ [{variant_name}] 训练完成！最佳验证集 DSC: {best_dsc:.4f}")
    return True

# ===================== 👑 满血评估引擎：注入6大核心指标 =====================
def evaluate_and_visualize(variant_name, params, ckpt_path):
    print(f"\n🚀 正在严谨评估变体 (6大指标全面盘点): {variant_name}")
    model = ADRNet(in_channels=1, out_channels=1, **params).to(DEVICE)
    ...
    with torch.no_grad():
        for view in views:
            ...
            print(f"  正在评估 {view} 切面全部 {len(test_files)} 个有效样本...")
            ...
            # 记录原始数据用于跨视图合并统计
            for k in metrics_keys: raw_results[view][k] = view_data[k]
    ...
    for k in metrics_keys:
        all_vals = []
        for v in views: all_vals.extend(raw_results[v][k])
        summary['global'][k] = float(np.nanmean(all_vals))
        summary['global'][f"{k}_std"] = float(np.nanstd(all_vals))
        
    print(f"✅ {variant_name} 评估完成！全局 DSC: {summary['global']['dsc']:.4f}")
    return summary

# ===================== 👑 Markdown 生成引擎：全指标注入 =====================
def generate_markdown_report(ablation_results, p_values, timestamp):
    md = f"""# ADR-Net 分割模型内部消融实验报告 (满血 6 指标版)

**实验时间**: {timestamp}
**实验设置**: 所有变体在完全相同的条件下训练 80 轮 (确保绝对公平性)。
**统计检验**: 与完整模型 (S7_ADRNet_Full) 的配对 t 检验，*: p<0.05, **: p<0.01, ***: p<0.001

## 1. 全局综合指标 (Global 6-Metrics)
| 模型变体 | 移除的模块说明 | DSC ↑ | IoU ↑ | Sens ↑ | Spec ↑ | HD95 ↓ (px) | ASSD ↓ (px) |
|---|---|---|---|---|---|---|---|
"""
    ours_name = 'S7_ADRNet_Full'
    metrics_keys = ['dsc', 'iou', 'sens', 'spec', 'hd95', 'assd']
    for variant_name in ABLATION_VARIANTS.keys():
        if variant_name not in ablation_results or ablation_results[variant_name] is None: continue
        ...
        if variant_name != ours_name and variant_name in p_values:
            for k in metrics_keys: stars[k] = get_p_stars(p_values[variant_name][k])
        
        if "Full" in variant_name: desc = "完整模型 (Ours)"
        elif "Baseline" in variant_name: desc = "基础 UNet"
        elif "Original_Attention" in variant_name: desc = "原始 Attention Gate"
        elif "wo_Multiscale" in variant_name: desc = "移除多尺度分支"
        elif "wo_Rate5" in variant_name: desc = "移除全局感受野分支 (Rate=5)"
        elif "wo_Dynamic_Weight" in variant_name: desc = "移除动态权重预测头"
        elif "wo_Asymmetric_Dilation" in variant_name: desc = "移除非对称感受野衰减"
        else: desc = variant_name
            
        md += f"| {variant_name} | {desc} | {res['dsc']:.4f}{stars['dsc']}±{res['dsc_std']:.4f} | {res['iou']:.4f}{stars['iou']}±{res['iou_std']:.4f} | {res['sens']:.4f}{stars['sens']}±{res['sens_std']:.4f} | {res['spec']:.4f}{stars['spec']}±{res['spec_std']:.4f} | {res['hd95']:.2f}{stars['hd95']}±{res['hd95_std']:.2f} | {res['assd']:.2f}{stars['assd']}±{res['assd_std']:.2f} |\n"
    
    md += "\n## 2. 详细统计显著性检验 P 值汇总\n| 对比组合 | DSC | IoU | Sens | Spec | HD95 | ASSD |\n|---|---|---|---|---|---|---|\n"
    for variant_name, p_vals in p_values.items():
        md += f"| ADR-Net Full vs {variant_name} | {p_vals['dsc']:.4e} | {p_vals['iou']:.4e} | {p_vals['sens']:.4e} | {p_vals['spec']:.4e} | {p_vals['hd95']:.4e} | {p_vals['assd']:.4e} |\n"
        
    md += "\n## 3. 各切面独立指标分布\n"
    for view in ['A2C', 'A3C', 'A4C']:
        md += f"### 切面: {view}\n| 模型变体 | DSC ↑ | IoU ↑ | Sens ↑ | Spec ↑ | HD95 ↓ | ASSD ↓ |\n|---|---|---|---|---|---|---|\n"
        for variant_name in ABLATION_VARIANTS.keys():
            if variant_name not in ablation_results or ablation_results[variant_name] is None: continue
            res = ablation_results[variant_name]['per_view'][view]
            md += f"| {variant_name} | {res['dsc']:.4f}±{res['dsc_std']:.4f} | {res['iou']:.4f}±{res['iou_std']:.4f} | {res['sens']:.4f}±{res['sens_std']:.4f} | {res['spec']:.4f}±{res['spec_std']:.4f} | {res['hd95']:.2f}±{res['hd95_std']:.2f} | {res['assd']:.2f}±{res['assd_std']:.2f} |\n"
        md += "\n"
    return md
...
def main():
    print("="*80)
    print("🏆 ADR-Net 分割模型内部消融实验 (6指标满血统计增强版)")
    print("="*80)
        ...
        if not os.path.exists(ckpt_path):
            if variant_name == 'S7_ADRNet_Full':
                ...
                if os.path.exists(main_ckpt):
                    import shutil; shutil.copy(main_ckpt, ckpt_path)
                    print(f"\n✅ 已克隆主模型权重至 {ckpt_path}")
                else: train_variant(variant_name, params, ckpt_path)
            else: train_variant(variant_name, params, ckpt_path)
    ...
    print(f"\n🔥 全指标消融实验成功收官！")
    print(f"📊 完美对齐 Table 1 格式的满血版 MD 报告已生成: {md_path}")
