"""
fusion.py - 最终安全修复版（修复seg_counts覆盖bug + 新增特征L2归一化）
"""
import os
import json
import torch
...
class GlobalPLMBManager:
    ...
    def _load_state(self):
        try:
            ...
            # ✅ 修复1：seg_counts赋值错误（原来会覆盖所有患者的计数）
            self.seg_counts = {}
            for pid, views in raw_counts.items():
                self.seg_counts[pid] = {}  # 先初始化患者级字典
