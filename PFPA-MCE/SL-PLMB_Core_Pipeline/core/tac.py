...
# ✅ 将信号滤波频率参数写死在这里，保证高内聚，防止依赖外部config报错
SIGNAL_PARAMS = {
    'fps': 30,
    'heart_rate_min': 50,
    'heart_rate_max': 120,
...
def time_frequency_filter(signal_1d, fps=30, abnormal_mask=None):
    """
    👑 核心时频域双重滤波器 (Time-Frequency Filter)
    利用 STFT 在频域剔除高频斑点噪声和低频运动伪影，并在时域屏蔽心律不齐区间。
    """
    ...
    # 1. 频域滤波 (Frequency-domain purification)
    ...
    # 2. 时域净化 (Time-domain interpolation for abnormal cycles)
